...
From the results of this project it was shown that the best fit is achieved when FERRE is configured to fit the models to the data with the effective temperature, surface gravity and metallicity using a 4th order polynomial. The table below shows the reduced χ2 for each configuration for FERRE. It is important to note that while there is a configuration with a marginally lower reduced χ2, it was not considered better fitting as the number of poorly fitted spectra increases significantly. 
